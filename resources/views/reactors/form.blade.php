...
@section('content')
    <div class="h-100vh">
        <form method="POST" class="center-form" @if (isset($reactor)) action="{{ route('reactors.update', $reactor) }}"
            @else
                action="{{ route('reactors.store') }}" @endif>
            @isset($reactor)
                @method('PUT')
            @endisset
            @csrf
            <div class="card text-center ">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('article') is-invalid  @enderror" name="article"
                            id="article" value="@if (isset($reactor)) {{ $reactor->article }}@else{{ old('article') }} @endif " required autofocus>
                @error('article')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid  @enderror" name="name" id="name"
                        value="@if (isset($reactor)) {{ $reactor->name }}@else{{ old('name') }} @endif" required>
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <textarea type="text" class="form-control @error('desc') is-invalid  @enderror" name="desc" id="desc">@if (isset($reactor)){{ $reactor->desc }} @else{{ old('desc') }}@endif
                </textarea>
                    @error('desc')
                        <div class="invalid-feedback">{!! $message !!}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <select name="type" class="form-control @error('type') is-invalid  @enderror" id="type" required>
                        <option value="Модульный" @if (isset($reactor))
                            @if ($reactor->type=='Модульный')
                            selected="selected" @endif
                            @endif>
                            Модульный</option>
                        <option value="Дисковый" @if (isset($reactor))
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-control @error('status') is-invalid  @enderror" id="status" required>
                        ...
                        <option value="Активен" @if (isset($reactor))
                            @if ($reactor->status=='Активен')
                            selected="selected" @endif
                            @endif>
                            Активен</option>

                        <option value="Не активен" @if (isset($reactor))  @if ($reactor->status=='Не активен')
                            selected="selected" @endif
                            @endif>
                            Не активен</option>
                        <option value="В работе" @if (isset($reactor))
                            @if ($reactor->status=='В работе')
                            selected="selected" @endif
                            @endif>
                            В работе</option>
                        <option value="Завершен" @if (isset($reactor))
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('countmodules') is-invalid  @enderror"
                        name="countmodules" id="countmodules" value="{{ old('countmodules', '20') }}" required>
                    @error('countmodules')
                        <div class="invalid-feedback">{!! $message !!}</div>
                    @enderror
                </div>
